...
\begin{task}[points=1]{Differenzen- und Diffenrenzialquotient anwenden}
    ...
    \begin{cpenumerate}[label=\alph*), itemsep=1em]
        \item Wie ist der Differenzenquotient definiert?
                \IfSolutionT{\\[0.2ex] Der Differenzenquotient ist die mittlere Steigung zwischen zwei Punkten $(x_1,y_1)$ und $(x_2,y_2)$ einer Funktion f(x). }
        \item Was unterscheidet den Diffenrenzialquotienten vom Differenzenquotienten?
                \IfSolutionT{\\[0.2ex] Der Diffenrenzialquotienten ist der Differenzenquotient eines Punktes $P_1=(x_1,y_1)$ auf einer Funktion f(x) dessen zweiter Punkt $(x_2,y_2)$ gegen diesen in­fi­ni­te­si­mal genau strebt. 
                Daurch kann die Steigung in Punkt $P_1$ beschrieben werden.}
        \item Wann in welchen Fällen sind Differenzialquotient und Differenzenquotient identisch über die gesamte Funktion?
                \IfSolutionT{\\[0.2ex] Dieser Fall lieght vor wenn die Funktion $f(x)$ eine lineare Funktion ist und dadurch $f'(x)$ eine Konstante ist. 
                Somit ist die Steigung in jedem Punkt identisch und die mittlere Steigung zwischen zwei Punkt entspricht dieser.}
        \item Differenzenquotient von: $f(x)=2x^3$ mit $x\in\mathbb{R} [0;2]$ 
                \IfSolutionT{\\[0.2ex] $\dfrac{\Delta y}{\Delta x} = \dfrac{f(x_2)-f(x_1)}{x_2-x_1} \\[0.2ex] \Rightarrow \dfrac{f(2)-f(0)}{2-0} =\dfrac{4-0}{2} = 2 $}
        \item Differenzialquotient von: $g(x)=x^2$ mit $x\to1$ 
                \IfSolutionT{\\[0.2ex] $\dfrac{dx}{dy} = \lim_{x_2\to x_1} \dfrac{f(x_2)-f(x_1)}{x_2-x_1} = \lim_{h\to0} \dfrac{f(x_1+h)-f(x_1)}{h} 
...
\begin{task}[points=9]{Ganzrationale Funktionen ableiten (Potenz-, Faktor- und Summenregel)}
    ...
    \end{grayInfoBox}
    \begin{cpenumerate}[label=\alph*), itemsep=1em]
        \item $f(x)=3x^4-53$ \IfSolutionT{\\[0.2ex] $f'(x)=12x^3$}
        \item $g(x)=0,2x^5-2,5x^4+3x^2-2,79x+23$ \IfSolutionT{\\[0.2ex] $g'(x)=x^4-10x^3+6x-2,79$}
        \item $h(t)=0,25x^4-0,5t^2+x^2-6t+5x-3$ \IfSolutionT{\\[0.2ex] $h'(t)=-t-6$}
        \item $i(x)=\dfrac{2x^6-8x^4}{x^3+2x^2}$ \IfSolutionT{\\[0.2ex] $i'(x)=6x^2-8x$}
        \item $w(u)=\dfrac{5u^6+au^4-bu^2}{2au^2*(3a-5b)}$ \IfSolutionT{\\[0.2ex] $w'(u)=(6a^2-5ab)^{-1}*(20u^3+2au)$}
        \item $v(l)=5l^{2,5}+l^2-2l^{-3,5}$ \IfSolutionT{\\[0.2ex] $v'(l)=12,5l^{1,5}+2l+7l^{-4,5}$}
    \end{cpenumerate}
\end{task}
\clearpage
% Aufgabe 3
    ...
    \begin{figure}[ht]
        ...
        \begin{subfigure}[t]{.24\textwidth}
            ...
            \begin{tikzpicture}
                ...
                \end{axis}
            \end{tikzpicture}
            \caption*{\tikz{\node[inner sep=0pt,draw, minimum width = 1.5cm, minimum height=.8cm]{};}}
        \end{subfigure}
        \begin{subfigure}[t]{.25\textwidth}
            \centering
            \begin{tikzpicture}
        ...
        \begin{subfigure}[t]{.25\textwidth}
            ...
            \begin{tikzpicture}
                ...
                \end{axis}
            \end{tikzpicture}
            \caption*{\tikz{\node[inner sep=0pt,draw, minimum width = 1.5cm, minimum height=.8cm]{\IfSolutionT{$G_g$}};}}
        \end{subfigure}
        \begin{subfigure}[t]{.24\textwidth}
            \centering
            \begin{tikzpicture}
        ...
        \begin{subfigure}[t]{.25\textwidth}
            ...
            \begin{tikzpicture}
                ...
                \end{axis}
            \end{tikzpicture}
            \caption*{\tikz{\node[inner sep=0pt,draw, minimum width = 1.5cm, minimum height=.8cm]{\IfSolutionT{$G_f$}};}}
        \end{subfigure}
        \begin{subfigure}[t]{.25\textwidth}
            \centering
            \begin{tikzpicture}
        ...
        \begin{subfigure}[t]{.24\textwidth}
            ...
            \begin{tikzpicture}
                ...
                \end{axis}
            \end{tikzpicture}
            \caption*{\tikz{\node[inner sep=0pt,draw, minimum width = 1.5cm, minimum height=.8cm]{\IfSolutionT{$G_h$}};}}
        \end{subfigure}
    \end{figure}
